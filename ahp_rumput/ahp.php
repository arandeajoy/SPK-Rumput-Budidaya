	...
	for($i=0;$i<count($id_kriteria);$i++){
		for($ii=0;$ii<count($id_kriteria);$ii++){
			if($i==$ii){
				...
				if($i < $ii){
					$q=mysql_query("select nilai from nilai_kriteria where id_kriteria1='".$id_kriteria[$i]."' and id_kriteria2='".$id_kriteria[$ii]."'");
...
function ahp_get_matrik_kriteria_rumput($id_kriteria){
	for($i=0;$i<count($id_kriteria);$i++){
		for($ii=0;$ii<count($id_kriteria);$ii++){
			if($i==$ii){
				$matrik[$i][$ii]=1;
...
function ahp_get_matrik_rumput($id_kriteria, $id_rumput){
	for($i=0;$i<count($id_rumput);$i++){
		for($ii=0;$ii<count($id_rumput);$ii++){
			if($i==$ii){
				$matrik[$i][$ii]=1;
			}else{
				if($i < $ii){
					$q=mysql_query("select nilai from nilai_alternatif where id_kriteria='".$id_kriteria."' and id_rumput1='".$id_rumput[$i]."' and id_rumput2='".$id_rumput[$ii]."'");
					if(mysql_num_rows($q)>0){
						$h=mysql_fetch_array($q);
						$nilai=$h['nilai'];
						$matrik[$i][$ii]=$nilai;

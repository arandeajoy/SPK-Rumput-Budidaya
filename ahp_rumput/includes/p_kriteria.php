...
if(mysql_num_rows($q) > 0){
	while($h=mysql_fetch_array($q)){
		...
		$id=$h['id_kriteria'];
		$allow_del=true;
		if(mysql_num_rows(mysql_query("select * from nilai_kriteria where id_kriteria1='".$h['id_kriteria']."' limit 0,1"))>0){$allow_del=false;}
		if(mysql_num_rows(mysql_query("select * from nilai_kriteria where id_kriteria2='".$h['id_kriteria']."' limit 0,1"))>0){$allow_del=false;}
